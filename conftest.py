...
def logged_in_main_page_driver():
    ...
    email_field = driver.find_element(*locators.RegisterPage.EMAIL_FIELD)
    email = RandomAccountProfileData.get_email()
    email_field.send_keys(email)
    name_field = driver.find_element(*locators.RegisterPage.NAME_FIELD)
    name = RandomAccountProfileData.get_name()
    name_field.send_keys(name)
    pass_field = driver.find_element(*locators.RegisterPage.PASS_FIELD)
    password = RandomAccountProfileData.get_pass()
    pass_field.send_keys(password)

    register_button = driver.find_element(*locators.RegisterPage.REGISTRATION_BUTTON)
    register_button.click()

    WebDriverWait(driver, 3).until(expected_conditions.url_to_be(url.LOGIN_PAGE))
    email_field = driver.find_element(*locators.LoginPage.EMAIL_FIELD)
    email_field.send_keys(email)
    pass_field = driver.find_element(*locators.LoginPage.PASS_FIELD)
    pass_field.send_keys(password)
    enter_button = driver.find_element(*locators.LoginPage.ENTER_BUTTON)
    enter_button.click()
